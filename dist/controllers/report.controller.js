...
exports.exportReport = exports.getConversionFunnel = exports.getLeadSourcesReport = exports.getSalesPerformance = exports.getCSRPerformance = exports.getMonthlyReport = exports.getDailyReport = exports.getDashboardStats = void 0;
...
const getDashboardStats = async (req, res) => {
    try {
        ...
        // Get counts
        const [totalLeads, todayLeads, hotLeads, conversions, teamStats] = await Promise.all([
            supabase_1.supabaseAdmin.from('leads').select('*', { count: 'exact', head: true }),
            supabase_1.supabaseAdmin.from('leads').select('*', { count: 'exact', head: true })
            ...
            supabase_1.supabaseAdmin.from('leads').select('*', { count: 'exact', head: true })
                .eq('status', 'converted'),
            supabase_1.supabaseAdmin.from('users').select(`
        id,
        name,
        leads:leads(count)
      `).eq('role', 'csr')
        ...
        const { data: recentActivities } = await supabase_1.supabaseAdmin
...
const getDailyReport = async (req, res) => {
    try {
        ...
        const { data: leads, error } = await supabase_1.supabaseAdmin
            .from('leads')
            .select('*')
...
const getMonthlyReport = async (req, res) => {
    try {
        ...
        const dailyBreakdown = Object.values(dailyData).sort((a, b) => a.date - b.date);
        // Calculate converted leads
        const convertedLeads = leads?.filter((l) => l.status === 'converted') || [];
        const totalRevenue = convertedLeads.reduce((sum, l) => sum + (l.deal_value || 0), 0);
        const stats = {
            year,
            month,
...
const getCSRPerformance = async (req, res) => {
    try {
      ...
      `)
        ...
        logger_utils_1.logger.error('CSR performance error:', error);
        res.status(500).json({ error: 'Failed to fetch CSR performance' });
    }
};
exports.getCSRPerformance = getCSRPerformance;
const getSalesPerformance = async (req, res) => {
    try {
        ...
        let query = supabase_1.supabaseAdmin
            ...
            .eq('status', 'converted');
        if (startDate) {
            query = query.gte('converted_at', new Date(startDate).toISOString());
        }
        if (endDate) {
            query = query.lte('converted_at', new Date(endDate).toISOString());
        }
...
const getLeadSourcesReport = async (req, res) => {
    try {
        ...
        logger_utils_1.logger.error('Lead sources report error:', error);
        res.status(500).json({ error: 'Failed to generate lead sources report' });
    }
};
exports.getLeadSourcesReport = getLeadSourcesReport;
const getConversionFunnel = async (req, res) => {
    try {
        ...
        logger_utils_1.logger.error('Conversion funnel error:', error);
...
const exportReport = async (req, res) => {
    try {
        ...
        const job = await redis_1.reportQueue.add({
            type,
            startDate,
            endDate,
            format: type
        ...
        logger_utils_1.logger.error('Export report error:', error);
