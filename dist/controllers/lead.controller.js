...
exports.addLeadActivity = exports.getLeadActivities = exports.convertLead = exports.assignLead = exports.deleteLead = exports.updateLead = exports.createPublicLead = exports.createLead = exports.getLeadById = exports.getLeads = void 0;
...
const getLeads = async (req, res) => {
    try {
        ...
        let query = supabase_1.supabaseAdmin.from("leads").select("*", { count: "exact" });
        if (status)
        ...
        logger_utils_1.logger.error("Get leads error:", error);
...
const getLeadById = async (req, res) => {
    try {
      ...
      `)
        ...
        logger_utils_1.logger.error("Get lead error:", error);
...
const createPublicLead = async (req, res) => {
    try {
        ...
        try {
            // Add to lead queue
            await redis_1.leadQueue.add("new-lead", {
                leadId: data.id,
                leadName: data.name,
                source: "website",
                email: data.email,
            ...
            // Add to email queue
            await redis_1.emailQueue.add("new-lead", {
                leadId: data.id,
                leadName: data.name,
                source: "website",
                email: data.email
        ...
        logger_utils_1.logger.error("Create public lead error:", error);
        res.status(500).json({
...
const updateLead = async (req, res) => {
    try {
        ...
        logger_utils_1.logger.error("Update lead error:", error);
...
const deleteLead = async (req, res) => {
    try {
        ...
        const { error } = await supabase_1.supabaseAdmin.from("leads").delete().eq("id", id);
        if (error)
            throw error;
        res.json({ message: "Lead deleted successfully" });
    }
    catch (error) {
        logger_utils_1.logger.error("Delete lead error:", error);
...
const assignLead = async (req, res) => {
    try {
        ...
        logger_utils_1.logger.info(`Lead ${id} assigned to user ${userId}`);
        res.json({
        ...
        logger_utils_1.logger.error("Assign lead error:", error);
...
const convertLead = async (req, res) => {
    try {
        ...
        await redis_1.emailQueue.add("lead_converted", {
            leadId: data.id,
            leadName: data.name,
            dealValue,
        });
        logger_utils_1.logger.info(`Lead ${id} converted with deal value ${dealValue}`);
        res.json({
        ...
        logger_utils_1.logger.error("Convert lead error:", error);
...
const getLeadActivities = async (req, res) => {
    try {
        ...
        logger_utils_1.logger.error("Get activities error:", error);
...
const addLeadActivity = async (req, res) => {
    try {
        ...
        logger_utils_1.logger.error("Add activity error:", error);
